...
qlib:https://pypi.org/project/pyqlib/#files
"""
import sys
import numpy as np
import pandas as pd

sys.path.append("D:\quant\promise\python-strategy\Alpha\\alpha191")
...
from lib.ops.rolling import _alpha191_143
...
try:
    ...
    print(
        "如果不想安装qlib, 请使用将 https://github.com/microsoft/qlib/tree/main/qlib/data/_libs 下的pyx文件编译"
    )
...
def alpha191_001(data, corr_period=6):
    ...
    data2 = ((data["close"] - data["open"]) / data["open"]).rank(axis=1,
                                                                 pct=True)
...
def alpha191_002(data):
    ...
    """
    alpha = -(((data["close"] - data["low"]) -
               (data["high"] - data["close"])) /
              (data["high"] - data["low"])).diff(periods=1)
    return alpha
...
def alpha191_003(data, sum_period=6):
    ...
    part1 = data["close"] - np.minimum(data["low"][condition2],
                                       delay1[condition2])
    part2 = data["close"] - np.maximum(data["high"][condition3],
                                       delay1[condition3])
...
def alpha191_004(data, roll_close_long=8, roll_close_short=2, roll_vol=20):
    ...
    alpha = (~data["close"].isna()).astype(float).replace(0,
                                                          np.nan)  # 全1, nan不变
...
def alpha191_007(data, period=3):
    ...
    [RANK(MAX(VWAP - CLOSE, 3)) + RANK(MIN(VWAP - CLOSE, 3))] * RANK(DELTA(VOLUME, 3))
    """
    rkmax = (data["vwap"] - data["close"]).rolling(period).max().rank(axis=1,
                                                                      pct=True)
    rkmin = (data["vwap"] - data["close"]).rolling(period).min().rank(axis=1,
                                                                      pct=True)
...
def alpha191_009(data, ewma_min=2, ewma_max=7):
	...
	7, 2)
    """
    part1 = (data["high"] + data["low"]) / 2 - (data["high"].shift(1) +
                                                data["low"].shift(1)) / 2
...
def alpha191_011(data, sum_period=6):
	...
	, 6)
    """
    alpha = (2 * data["close"] - data["low"] - data["high"]) / (data["high"] -
                                                                data["low"])
...
def alpha191_012(data, mean_period=10):
    ...
    alpha = alpha * (
        -1 * (data["close"] - data["vwap"]).abs().rank(axis=1, pct=True))
    return alpha
...
def alpha191_016(data, coll_period=5):
	...
	, 5)
    """
    alpha = data["volume"].rank(axis=1, pct=True).rolling(coll_period).corr(
        data["vwap"].rank(axis=1, pct=True))
...
def alpha191_017(data, max_period=15, delta_period=5):
    ...
    """
    alpha = (data["vwap"] - data["vwap"].rolling(max_period).max()).rank(
        axis=1, pct=True)
...
def alpha191_021(data, period: int = 6, use_vwap: bool = True):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = data[_p].rolling(period).mean(
    )  # .rolling(period).apply(lambda x: np.polyfit(seq, x, 1)[0])
...
def alpha191_023(data, ma_period=20, use_vwap: bool = True):
    ...
    alpha = SMA(part1, ma_period, 1) / (SMA(part1, ma_period, 1) +
                                        SMA(part2, ma_period, 1)) * 100
    return alpha
...
def alpha191_025(data,
    ...
    part2 = (data["volume"] / data["volume"].rolling(vol_period).mean()).ewm(
        alpha=1 / 9).mean().rank(axis=1, pct=True)
...
def alpha191_029(data, use_vwap: bool = True):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = (data[_p] - data[_p].shift(6)) / data[_p].shift(6) * np.log(
        data["volume"])
    return alpha
...
def alpha191_031(data, period=12, use_vwap: bool = True):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = (data[_p] - data[_p].rolling(period).mean()
             ) / data[_p].rolling(period).mean() * 100
    return alpha
...
def alpha191_032(data, period=3):
    ...
    """
    corr = data["high"].rank(axis=1, pct=True).rolling(period).corr(
        data["volume"].rank(axis=1, pct=True))
...
def alpha191_035(data, ma_period1=15, ma_period2=7, corr_period=17):
    ...
    """
    part1 = (data["open"] - data["open"].shift(1)).ewm(
        alpha=1 / ma_period1).mean().rank(axis=1, pct=True)
    part2 = data["volume"].rolling(corr_period).corr(
        (data["open"] * 0.65 +
         data["close"] * 0.35)).ewm(alpha=1 / ma_period2).mean().rank(axis=1,
                                                                      pct=True)
...
def alpha191_036(data, corr_period=6, sum_period=2):
    ...
    """
    corr = data["volume"].rank(axis=1, pct=True).rolling(corr_period).corr(
        data["vwap"].rank(axis=1, pct=True))
...
def alpha191_037(data, sum_period=5, delay_period=10, use_vwap: bool = True):
    ...
    alpha = data["open"].rolling(sum_period).sum() * ret.rolling(
        sum_period).sum()
...
def alpha191_039(data):
    ...
    alpha -= RANK(
        DECAYLINEAR(
            CORR((data["vwap"] * 0.3 + data["open"] * 0.7),
...
def alpha191_041(data, delta_period=3, max_period=5):
    ...
    """
    alpha = data["vwap"].diff(delta_period).rolling(max_period).max().rank(
        axis=1, pct=True) * -1
    return alpha
...
def alpha191_044(data):
    ...
    """
    alpha = TS_RANK(
        DECAYLINEAR(CORR(data["low"], MEAN(data["volume"], 10), 7), 6), 4)
...
def alpha191_045(data, corr_period=15, mean_period=150):
    ...
    """
    alpha = (data["close"] * 0.6 + data["open"] * 0.4).diff(1).rank(axis=1,
                                                                    pct=True)
    alpha *= data["vwap"].rolling(corr_period).corr(
        data["volume"].rolling(mean_period).mean())
    return alpha
...
def alpha191_048(data, use_vwap=False):
    ...
    alpha = alpha.rank(axis=1, pct=True) * data["volume"].rolling(
        5).sum() / data["volume"].rolling(20).sum()
    return alpha
...
def alpha191_049(data, period=12):
    ...
    """
    condition = (data["high"] + data["low"]) >= (data["high"].shift(1) +
                                                 data["low"].shift(1))
...
def alpha191_050(data, period=12):
    ...
    """
    condition1 = (data["high"] + data["low"]) <= (data["high"].shift(1) +
                                                  data["low"].shift(1))
    condition2 = (data["high"] + data["low"]) >= (data["high"].shift(1) +
                                                  data["low"].shift(1))
    part = np.maximum(np.abs(data["high"] - data["high"].shift(1)),
...
def alpha191_053(data, period=12, use_vwap=False):
    ...
    alpha = (data[_p] > data[_p].shift(1)
             ).astype(float).rolling(period).sum() / period * 100
    return alpha + na_map
...
def alpha191_055(data, period=20, use_vwap=False):
    ...
    alpha = 16 * (data[_p] - DELAY(data[_p], 1) +
                  (data[_p] - data["open"]) / 2 + DELAY(data[_p], 1) -
                  DELAY(data["open"], 1))
    alpha = alpha / IFELSE((part1 > part2) & (part1 > part3), var1,
                           IFELSE(
                               (part2 > part3) & (part2 > part1), var2, var3))
...
def alpha191_058(data, period=20, use_vwap=False):
    ...
    _p = "vwap" if use_vwap else "close"
    na_map = data["open"].isna().astype(float).replace(1., np.nan)
    alpha = COUNT(data[_p] > data[_p].shift(1), period,
                  na_map=na_map) / period * 100
    return alpha + na_map
...
def alpha191_061(data):
    ...
    alpha = np.maximum(
        ...
        RANK(
            DECAYLINEAR(RANK(CORR((data["low"]), MEAN(data["volume"], 80), 8)),
                        17)))
    return alpha
...
def alpha191_064(data):
    ...
    """
    alpha = np.maximum(
        RANK(DECAYLINEAR(CORR(RANK(data["vwap"]), RANK(data["volume"]), 4),
                         4)),
        RANK(
            DECAYLINEAR(
                MAX(
                    CORR(RANK(data["close"]), RANK(MEAN(data["volume"], 60)),
                         4), 13), 14)))
    return alpha
...
def alpha191_068(data, period=15):
    ...
    """
    alpha = (data["high"] + data["low"]) / 2 - (data["high"].shift(1) +
                                                data["low"].shift(1)) / 2
...
def alpha191_069(data, period=20):
    ...
    """
    dtm = IFELSE(
        data["open"] <= DELAY(data["open"], 1), 0,
        np.maximum(data["high"] - data["open"],
                   data["open"] - DELAY(data["open"], 1)))
    dbm = IFELSE(
        data["open"] >= DELAY(data["open"], 1), 0,
        np.maximum(data["open"] - data["low"],
                   data["open"] - DELAY(data["open"], 1)))
    alpha = IFELSE(
        SUM(dtm, period) > SUM(dbm, period),
        (SUM(dtm, period) - SUM(dbm, period)) / SUM(dtm, period),
        IFELSE(
            SUM(dtm, period) == SUM(dbm, period), 0,
...
def alpha191_074(data, sum_period=20, corr_period1=7, corr_period2=6):
    ...
    """
    alpha = RANK(
        CORR(SUM(data["low"] * 0.35 + data["vwap"] * 0.65, sum_period),
             SUM(MEAN(data["volume"], 40), sum_period), corr_period1)) + RANK(
                 CORR(RANK(data["vwap"]), RANK(data["volume"]), corr_period2))
    return alpha
...
def alpha191_077(data):
    ...
    """
    alpha = RANK(
        DECAYLINEAR((((data["high"] + data["low"]) / 2 + data["high"]) -
                     (data["vwap"] + data["high"])), 20))
    alpha = np.minimum(
        alpha,
        RANK(
            DECAYLINEAR(
                CORR(((data["high"] + data["low"]) / 2),
...
def alpha191_080(data, period=5):
    ...
    """
    alpha = (data["volume"] -
             data["volume"].shift(period)) / data["volume"].shift(period) * 100
    return alpha
...
def alpha191_083(data, cov_period=5):
    ...
    """
    alpha = -1 * RANK(
        COVARIANCE(RANK(data["high"]), RANK(data["volume"]), cov_period))
    return alpha
...
def alpha191_086(data, long_period=20, short_period=8, use_vwap=False):
    ...
    part1 = (data[_p].shift(long_period) -
             data[_p].shift(short_period)) / short_period
...
def alpha191_087(data, period=7):
    ...
    * -1
    @res:
    """
    alpha = (RANK(DECAYLINEAR(DELTA(data["vwap"], 4), period)) + TS_RANK(
        DECAYLINEAR(
            ((((data["low"] * 0.9) + (data["low"] * 0.1)) - data["vwap"]) /
             (data["open"] -
              ((data["high"] + data["low"]) / 2) + 1e-7)), 11), period)) * -1
    return alpha
...
def alpha191_090(data, corr_period=5):
    ...
    @res:
    """
    alpha = RANK(CORR(RANK(data["vwap"]), RANK(data["volume"]),
                      corr_period)) * -1
    return alpha
...
def alpha191_092(data, decay_period1=3, decay_period2=5, rank_period=15):
    ...
    alpha = MAX(
        RANK(
            DECAYLINEAR(DELTA((data["close"] * 0.35 + data["vwap"] * 0.65), 2),
                        decay_period1)),
        TS_RANK(
            DECAYLINEAR(
                np.abs(CORR(MEAN(data["volume"], 180), data["close"], 13)),
                decay_period2), rank_period)) * -1
    return alpha
...
def alpha191_093(data, sum_period=20):
    ...
    @res:
    """
    alpha = MAX(data["open"] - data["low"],
                data["open"] - data["open"].shift(1))
    alpha = SUM(IFELSE(data["open"] >= data["open"].shift(1), 0, alpha),
                sum_period)
...
def alpha191_096(data, minmax_period=9, ma_period=3):
    ...
    """
    alpha = SMA(
        SMA((data["close"] - TS_MIN(data["low"], minmax_period)) /
            (TS_MAX(data["high"], minmax_period) -
             TS_MIN(data["low"], minmax_period)) * 100, ma_period, 1),
        ma_period, 1)
    return alpha
...
def alpha191_098(data, period=100, use_vwap=False):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    condition = DELTA(SUM(data[_p], period) / period,
                      period) / data[_p].shift(period)
    alpha = IFELSE(condition <= 0.05,
                   -1 * (data[_p] - TS_MIN(data[_p], period)),
                   -1 * DELTA(data[_p], 3))
    return alpha
...
def alpha191_099(data, cov_period=5, use_vwap=False):
    ...
    _p = "vwap" if use_vwap else "close"
    alpha = -1 * RANK(
        COVARIANCE(RANK(data[_p]), RANK(data["volume"]), cov_period))
    return alpha
...
def alpha191_101(data,
    ...
    """
    alpha = (RANK(
        CORR(data["close"], SUM(MEAN(data["volume"], ma_period1), ma_period2),
             corr_period1)) < RANK(
                 CORR(RANK((data["high"] * 0.1 + data["vwap"] * 0.9)),
                      RANK(data["volume"]), corr_period2))) * -1
    return alpha
...
def alpha191_104(data, corr_period=5, std_period=20, use_vwap=False):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = -1 * (DELTA(CORR(data["high"], data["volume"], corr_period),
                        corr_period) * RANK(STD(data[_p], std_period)))
    return alpha
...
def alpha191_108(data):
    ...
    alpha = (RANK((data["high"] - TS_MIN(data["high"], 2)))**RANK(
        CORR((data["vwap"]), (MEAN(data["volume"], 120)), 6))) * -1
    return alpha
...
def alpha191_113(data, period=20, corr_period=20, use_vwap=False):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = -1 * (RANK(SUM(DELAY(data[_p], 5), period) / period) * CORR(
        data[_p], data["volume"], corr_period) * RANK(
            CORR(SUM(data[_p], 5), SUM(data[_p], period), corr_period)))
    return alpha
...
def alpha191_115(data):
    ...
    """
    alpha = (RANK(
        CORR((data["high"] * 0.9 + data["close"] * 0.1),
             MEAN(data["volume"], 30), 10))**RANK(
                 CORR(TS_RANK((data["high"] + data["low"]) / 2, 4),
...
def alpha191_116(data, period=20, use_vwap=False):
    ...
    idx = data[_p].index
    alpha = data[_p].apply(
        lambda x: pd.Series(rolling_slope(x.values, period)))
    alpha.index = idx
    return alpha
...
def alpha191_118(data, period=20):
    ...
    @res:
    """
    alpha = SUM(data["high"] - data["open"], period) / SUM(
        data["open"] - data["low"], period) * 100
    return alpha
...
def alpha191_120(data):
    ...
    @res:
    """
    alpha = RANK(data["vwap"] - data["close"]) / RANK(data["vwap"] +
                                                      data["close"])
    return alpha
...
def alpha191_121(data):
    ...
    alpha = (RANK(data["vwap"] - TS_MIN(data["vwap"], 12))**TS_RANK(
        CORR(TS_RANK(data["vwap"], 20), TS_RANK(MEAN(data["volume"], 60), 2),
             18), 3)) * -1
    return alpha
...
def alpha191_123(data, period=20):
    ...
    """
    alpha = RANK(
        CORR(SUM((data["high"] + data["low"]) / 2, period),
             SUM(MEAN(data["volume"], 60), period), 9))
    na_map = alpha.isna().astype(float).replace(1., np.nan)
    alpha = (alpha < RANK(CORR(data["low"], data["volume"], 6))).replace(
        False, -1) * -1
    return alpha + na_map
...
def alpha191_124(data, period=30):
    ...
    @res:
    """
    alpha = (data["close"] - data["vwap"]) / DECAYLINEAR(
        RANK(TS_MAX(data["close"], period)), 2)
    return alpha
...
def alpha191_127(data, period=12):
    ...
    @res:
    """
    alpha = (MEAN((100 * (data["close"] - TS_MAX(data["close"], period)) /
                   (TS_MAX(data["close"], period)))**2, period))**0.5
    return alpha
...
def alpha191_131(data, period=18, vol_period=50):
    ...
    @res:
    """
    alpha = (RANK(DELTA(data["vwap"], 1))**TS_RANK(
        CORR(data["close"], MEAN(data["volume"], vol_period), period), period))
    return alpha
...
def alpha191_134(data, period=12, use_vwap=False):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = (data[_p] - DELAY(data[_p], period)) / DELAY(
        data[_p], period) * data["volume"]
    return alpha
...
def alpha191_136(data, corr_period=10):
    ...
    """
    ret = data["close"] / DELAY(data["close"], 1) - 1
    alpha = (-1 * RANK(DELTA(ret, 3))) * CORR(data["open"], data["volume"],
                                              corr_period)
    return alpha
...
def alpha191_138(data):
    ...
    alpha = (RANK(
        DECAYLINEAR(DELTA(
            (((data["low"] * 0.7) + (data["vwap"] * 0.3))), 3), 20)) - TS_RANK(
                DECAYLINEAR(
                    TS_RANK(
                        CORR(TS_RANK(data["low"], 8),
                             TS_RANK(MEAN(data["volume"], 60), 17), 5), 19),
                    16), 7)) * -1
    return alpha
...
def alpha191_140(data):
    ...
    alpha = np.minimum(
        RANK(
            DECAYLINEAR(((RANK(data["open"]) + RANK(data["low"])) -
                         (RANK(data["high"]) + RANK(data["close"]))), 8)),
        TS_RANK(
            DECAYLINEAR(
                CORR(TS_RANK(data["close"], 8),
                     TS_RANK(MEAN(data["volume"], 60), 20), 8), 7), 3))
    return alpha
...
def alpha191_141(data):
    ...
    """
    alpha = (
        RANK(CORR(RANK(data["high"]), RANK(MEAN(data["volume"], 15)), 9)) * -1)
    return alpha
...
def alpha191_142(data):
    ...
    @res:
    """
    alpha = (((-1 * RANK(TS_RANK(data["close"], 10))) *
...
def alpha191_143(data, use_vwap=False):
    ...
    alpha = pd.DataFrame(_alpha191_143(data[_p].values, delay.values),
                         index=delay.index,
...
def alpha191_144(data):
    ...
    @res:
    """
    part = np.abs(data["close"] / DELAY(data["close"], 1) - 1) / np.log(
        data["amount"])
    alpha = IFELSE(data["close"] < DELAY(data["close"], 1), part, 0)
    na_map = alpha.isna().astype(float).replace(1., np.nan)
    alpha = SUM(alpha, 20) / COUNT(
        data["close"] < DELAY(data["close"], 1), 20, na_map=na_map)
    return alpha
...
def alpha191_145(data):
    ...
    @res:
    """
    alpha = (MEAN(data["volume"], 9) - MEAN(data["volume"], 26)) / MEAN(
        data["volume"], 12) * 100
    return alpha
...
def alpha191_146(data, period=61, use_vwap=False):
    ...
    alpha = MEAN(part_ma, 20) * (part_ma) / SMA(
        (part - (part_ma))**2, period, 2)
    return alpha
...
def alpha191_147(data, period=12, use_vwap=False):
    ...
    alpha = MEAN(
        data[_p],
        period).apply(lambda x: pd.Series(rolling_slope(x.values, period)))
    alpha.index = idx
    return alpha
...
def alpha191_148(data):
    ...
    na_map = alpha.isna().astype(float).replace(1., np.nan)
    alpha = (alpha < RANK(
        (data["open"] - TS_MIN(data["open"], 14)))).replace(False, -1) * -1
    return alpha + na_map
...
def alpha191_150(data):
    ...
    @res:
    """
    alpha = (data["close"] + data["high"] + data["low"]) / 3 * np.log(
        data["volume"])
    return alpha
...
def alpha191_153(data, use_vwap=False):
    ...
    """
    _p = "vwap" if use_vwap else "close"
    alpha = (MEAN(data[_p], 3) + MEAN(data[_p], 6) + MEAN(data[_p], 12) +
             MEAN(data[_p], 24)) / 4
    return alpha
...
def alpha191_154(data):
    ...
    @res:
    """
    alpha = ((data["vwap"] - TS_MIN(data["vwap"], 16))
             )  # < (CORR(data["vwap"], MEAN(data["volume"], 180), 18))
    na_map = alpha.isna().astype(float).replace(1., np.nan)
    alpha = (alpha < CORR(data["vwap"], MEAN(data["volume"], 180),
                          18)).replace(False, -1)
    return alpha + na_map
...
def alpha191_156(data, period=3):
    ...
    alpha = np.maximum(
        ...
        RANK(
            DECAYLINEAR(
                ((DELTA(((data["open"] * 0.15) + (data["low"] * 0.85)), 2) /
                  ((data["open"] * 0.15) + (data["low"] * 0.85))) * -1),
                period)))
    alpha = alpha * -1
    return alpha
    ...
    alpha = MEAN(
        MAX(
            MAX((data["high"] - data["low"]),
                np.abs(DELAY(data["close"], 1) - data["high"])),
...
def alpha191_162(data, period=12, use_vwap=False):
    ...
    alpha = (part2 / part3 * 100 -
             part4) / (TS_MAX(part2 / part3 * 100, period) - part4)
    return alpha
...
def alpha191_163(data, period=20):
    ...
    alpha = RANK(
        ((((-1 * ret) * MEAN(data["volume"], period)) * data["vwap"]) *
         (data["high"] - data["close"])))
    return alpha
...
def alpha191_164(data, use_vwap=False):
    ...
    alpha = SMA(
        IFELSE(condition, 1 / diff, 1) -
        TS_MIN(IFELSE(condition, 1 / diff, 1), 12) /
...
def alpha191_165(data, period=48, use_vwap=False):
    ...
    alpha = TS_MAX(SUM(diff, 48),
                   48) - TS_MIN(SUM(diff, 48), 48) / STD(data[_p], period)
    return alpha
...
def alpha191_166(data):
    ...
    alpha = 5 * SUM(part - 1 - MEAN(part - 1, 20), 20) / (SUM(
        MEAN(part, 20)**2, 20))**1.5
    return alpha
...
def alpha191_170(data):
    ...
    alpha *= (data["high"] *
              RANK(data["high"] - data["close"])) / (SUM(data["high"], 5) / 5)
...
def alpha191_172(data):
    ...
    tr = MAX(
        MAX(data["high"] - data["low"],
            np.abs(data["high"] - DELAY(data["close"], 1))),
...
def alpha191_173(data, period=13, use_vwap=False):
    ...
    alpha = 3 * ma - 2 * SMA(ma, period, 2) + SMA(
        SMA(np.log(data[_p]), period, 2), period, 2)
    return alpha
...
def alpha191_176(data, period=12, corr_period=6):
    ...
    """
    alpha = CORR(
        RANK((data["close"] - TS_MIN(data["low"], period)) /
...
def alpha191_178(data):
    ...
    @res:
    """
    alpha = (data["close"] - DELAY(data["close"], 1)) / DELAY(
        data["close"], 1) * data["volume"]
    return alpha
...
def alpha191_180(data):
    ...
    """
    alpha = IFELSE(
        MEAN(data["volume"], 20) < data["volume"],
        (-1 * TS_RANK(np.abs(DELTA(data["close"], 7)), 60)) *
        np.sign(DELTA(data["close"], 7)), -data["volume"])
    return alpha
...
def alpha191_181(data):
    ...
    alpha = SUM(alpha.sub(part**2, axis="index"), 20).div(SUM(part**3, 20),
                                                          axis="index")
    return alpha
...
def alpha191_186(data, period=14, ma_period=6):
    ...
    alpha = (MEAN(part3, ma_period) +
             DELAY(MEAN(part3, ma_period), ma_period)) / 2
    return alpha
...
def alpha191_187(data, period=20):
    ...
    alpha = SUM(
        IFELSE(
            data["open"] <= DELAY(data["open"], 1), 0,
            MAX((data["high"] - data["open"]),
                (data["open"] - DELAY(data["open"], 1)))), period)
    return alpha
...
def alpha191_188(data, period=11):
    ...
    @res:
    """
    alpha = ((data["high"] - data["low"] -
              SMA(data["high"] - data["low"], period, 2)) /
...
def alpha191_190(data, use_vwap=False):
    ...
    alpha = np.log((COUNT(part1 > part2, 20, na_map=na_map) - 1) *
...
def alpha191_191(data):
    ...
    @res:
    """
    alpha = ((CORR(MEAN(data["volume"], 20), data["low"], 5) +
...
class Alpha191(FactorBase):
    ...
    def _prepare_data(self, input_data: Dict[str, pd.DataFrame]):
        """
            inputdata col:date\code\open\high\low\close\volume\amount\turn
        """
        fields = list(input_data.keys())
        columns = input_data[fields[0]].columns
        shape = input_data[fields[0]].shape
        assert "date" not in columns, "date is in columns, the input data must be indexed by date"
        assert shape[
            0] > 300 + self.diff_days, f"the input data must have more than {300 + self.diff_days} rows"
        self.data = {}
        for field in fields:
            assert input_data[
                field].shape == shape, f"{field} shape is not equal to {fields[0]}"
            self.data[field] = input_data[field].reindex(
    ...
    def _split_data(self):
        ...
        for i in range(len(group)):
            split_data.append({
                field: self.data[field].iloc[-group[i]:]
            ...
            for fac_name, fac_func in self.alpha191_funcs.items():
                if not fac_func.unfinished:
                    if self.verbose:
                        print(f"cal {fac_name}, pid: {os.getpid()}")
                    try:
                        res[fac_name] = fac_func(
                            self._map_depend(fac_func.max_depend))
                    except Exception as e:
                        print(f"{fac_name} error {e}")
        return res
