...
from lib.ops.rolling import _wma, _max_distance, _min_distance, _alpha191_143
...
def IFELSE(condition: pd.DataFrame, A: Union[pd.DataFrame, Any],
           B: Union[pd.DataFrame, Any]):
    """
    条件选择
    """
...
def HIGHDAY(A: pd.DataFrame, N: int, zero_diff=False):
    ...
    na_map = A.isna().astype(float).replace(1., np.nan)
    A = A.fillna(-np.inf)
    return pd.DataFrame(_max_distance(A.values, N), index=idx,
                        columns=col) - int(zero_diff) + na_map
...
def LOWDAY(A: pd.DataFrame, N: int, zero_diff=False):
    ...
    na_map = A.isna().astype(float).replace(1., np.nan)
    A = A.fillna(+np.inf)
    return pd.DataFrame(_min_distance(A.values, N), index=idx,
                        columns=col) - int(zero_diff) + na_map
...
def VaR(price: pd.DataFrame, N: int, p: float = 0.05, method: str = "param"):
    ...
    if method == "historical":
        ...
        return ret.rolling(
            N).mean() + ret.rolling(N).std() * norm.ppf(p) + na_map
