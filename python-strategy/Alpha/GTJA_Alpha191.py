...
class GTJA_191:
    ...
    def __init__(self, end_date, index,stocks_pool,count = 250,fq = 'pre'):
        ...
        price = get_price(stocks_pool, None, end_date, '1d', ['open','close','low','high','avg','pre_close','volume','money'], False, fq, count)
        benchmark_price = get_price(index, None, end_date, '1d',['open','close','low','high','avg','pre_close','volume'], False, fq, count,)
        ...
        self.prev_close = price.loc['pre_close',:,:].dropna(axis=1,how='any')
    ...
    def alpha_001(self):
        ...
        data2 = ((self.close - self.open_price)/self.open_price).rank(axis=1,pct=False)
    ...
    def alpha_002(self):
        ...
        '''
        # 与joinQuant测试 结果一致
        result=-1 * (((self.close-self.low)-(self.high-self.close))/((self.high-self.low))).diff()
    ...
    def alpha_003(self):
        ...
        part3 = (self.close-np.maximum(delay1[condition3],self.high[condition3])).iloc[-6:,:]
    ...
    def alpha_004(self):
        ...
        condition1=((self.close.rolling(8).sum()/8 + self.close.rolling(8).std())< self.close.rolling(2).sum()/2) # 2日均价突破
        condition2=((self.close.rolling(8).sum()/8 - self.close.rolling(8).std())> self.close.rolling(2).sum()/2) # 2日均价跌破
    ...
    def alpha_025(self):
        ...
        temp1=temp.iloc[-9:,:]
        seq=[2*i/(n*(n+1)) for i in range(1,n+1)] # sum(1:(n+1)) => n*(n+1)/2
        weight=np.array(seq)
        # print(temp1.sum())
...
if __name__ == "__main__":
    ...
    yesterday_trdae_date = trade_list[np.argwhere(trade_list < dt.date.today())[-1]][0]  # 获取昨日交易日
    end_date = yesterday_trdae_date
    end_date = dt.date(2020,5,28)
    # security = get_index_stocks(index).index.to_list()
    security = ['000968.XSHE',
                '002130.XSHE',
                '002167.XSHE',
                '002370.XSHE',
                '002832.XSHE',
    ...
    # 使用前复权
    alpha191 = GTJA_191(end_date=end_date,index= index,stocks_pool=security,count = 350,fq = 'post')
